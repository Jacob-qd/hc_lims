...
| 4. 测试覆盖 | > 90% | **50.21%** 行 / **30.23%** 分支 / **34.24%** 函数 | ❌ 未达成 |
| 5. 设计文档符合度 | > 90% | **~65%** | ❌ 未达成 |
...
- **测试状态**: `npm run test` ✅ 通过 (258 测试 / 51 测试文件全部通过)
- **Lint/格式**: 无配置冲突

### 2.2 测试覆盖详情

```
Statements   : 46.11% ( 1393/3021 )
Branches     : 30.23% ( 534/1766 )
Functions    : 34.24% ( 413/1206 )
Lines        : 50.21% ( 1272/2533 )
```
...
- WorkflowPage: 27.54%
- ReportsPage: 32.18%
- ReportEnginePage: 31.79%
- BackupPage: 39.65%
- COCPage: 36.25%
- CertificatePage: 41.79%

### 2.3 页面测试覆盖

- **全部 40 个页面**均已有测试文件 (`src/test/*.test.tsx`)
- **16 个页面**拥有交互级测试（模态框、表单提交、Tab 切换、Drawer 打开等）
- **24 个页面**目前仅有渲染测试（mount 不报错）
...
`src/mocks/handlers.ts` 已覆盖 **~45 个端点**，包括：
...
要达到 **90%** 行覆盖，还需覆盖 **~1010 行**代码。重点缺口：

1. **ReportsPage** (1688 行，当前 32.18%)
   - 报告编制 Drawer、PDF 预览、签名模态框、审核模态框、批注面板
2. **WorkflowPage** (1032 行，当前 27.54%)
   - FlowDesigner 画布拖拽/连线/删除、设计器 Modal、实例详情 Drawer
3. **ReportEnginePage** (712 行，当前 31.79%)
   - 模板设计器步骤切换、数据源配置、图表配置
4. **SamplesPage** (541 行，当前 59.25%)
   - Wizard 4 步流程、容器管理、检测项目选择、批量导入

**估算**: 要写 ~150–200 个额外测试用例，约 **2–3 人天**工作量。
...
1. **短期（1 周）**：集中补充 ReportsPage + WorkflowPage + ReportEnginePage 的交互测试，可将行覆盖提升至 **70%+**。
...
- **main 分支**: `2fd8a85` — 包含全部合并特性 + 测试 + 构建修复
- **已推送 origin**: ✅
- **已关闭 beads**: hc-1km, hc-eh5, hc-o3b, hc-ah2
- **待处理 beads**: hc-69b（测试覆盖，进度 ~55%）
