...
| Story | 描述 | 状态 | 验证 |
|:---:|------|:---:|------|
| US1 | 样品登记向导 | ✅ | 4步向导 + 容器动态添加 |
| US2 | 分样管理 | ✅ | 分样Tab + 子样品展示 + 体积校验测试 |
| US3 | 留样管理 | ✅ | 留样Tab + 到期颜色 + 处置Modal + 电子签名 |
| US4 | 位置追踪 | ✅ | 库位管理Card + 结构化位置 + 移动记录 |
| US5 | 异常处理 | ✅ | 异常样品Card + 有条件接收 + 拍照记录 |
| US6 | 批量导入 | ✅ | 批量导入按钮 + CSV解析 + 校验逻辑测试 |

### 测试覆盖

| 测试文件 | 用例数 | 覆盖 |
|------|:---:|------|
| SampleManagement.test.ts | 16 | US2-US6 |
| P2Features.test.ts | 4 | 分样体积校验 + 留样到期 |
| BusinessLogic.test.ts | 3 | 样品状态流转 |
