...
### R1 修正
- ✅ 派工规则实现可编辑 Modal
- ✅ 负载数据从 mock 改为动态计算 (遍历 tasks 统计各检测员未完成任务数)
- ✅ 增加「预览派工结果」步骤 (Modal展示分配明细, 确认后执行)
- ✅ 看板卡片支持 dnd-kit 拖拽

### R2 修正
- 🔴 派工结果审计: 记录自动/手动分配的操作人+时间+分配详情
- 🔴 状态联动: 任务分配→通知检测员; 任务完成→触发报告生成检查
- 🔴 并发控制: 同一任务不可同时分配给两人 (乐观锁 version 字段)
