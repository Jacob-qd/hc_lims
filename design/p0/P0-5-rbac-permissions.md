...
### R1 修正
- ✅ 权限矩阵单元格支持点击切换 ✓/✗
- ✅ ProfilePage 密码修改关联策略校验 (长度/复杂度/历史)
- ✅ 委托审批实现状态变更 (approve→active, reject→通知申请人)
- ✅ 增加「我的权限」查看入口 (ProfilePage 或独立Modal)

### R2 修正
- 🔴 权限变更审计: 每次角色/权限修改记录操作人+时间+变更内容
- 🔴 权限变更通知: 发送站内通知给被变更用户
- 🔴 密码过期强制修改: 登录时检测, 过期强制跳转修改页
- 🔴 会话超时: 30分钟无操作自动登出
- 🔴 统一数据获取层: usePermissions() 解耦 Mock
