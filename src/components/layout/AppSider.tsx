...
// 根据角色过滤菜单
const filterMenuByRole = (items: LooseAny[], role: string): LooseAny[] => {
  return items
    .filter(item => {
...
export const AppSider: React.FC = () => {
  ...
  const getSelectedKey = () => {
    const flatItems = filteredMenu.flatMap((item: LooseAny) => item.children ? [item, ...item.children] : [item]);
    const match = flatItems.find((item: LooseAny) => item.key && location.pathname.startsWith(item.key));
