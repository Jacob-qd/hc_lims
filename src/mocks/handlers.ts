...
export const handlers = [
  ...
  http.post(apiUrl('/workflow/definitions/:id/deploy'), ({ params }) => {
    ...
    def.status = 'deployed';
    def.updatedAt = new Date().toISOString().replace('T', ' ').slice(0, 19);
  ...
  http.post(apiUrl('/workflow/instances'), async ({ request }) => {
    ...
    const newInst = {
      ...
      status: 'running',
      currentNodes: def?.nodes.filter((n: any) => n.type === 'start').map((n: any) => n.id) || [],
      currentNodeNames: def?.nodes.filter((n: any) => n.type === 'start').map((n: any) => n.name) || [],
      assignees: [],
  ...
  http.post(apiUrl('/workflow/instances/:id/urge'), ({ params }) => {
    ...
    return HttpResponse.json({ code: 200, message: '催办成功', data: inst });
  }),
  ...
  http.post(apiUrl('/workflow/instances/:id/terminate'), async ({ params, request }) => {
    ...
    return HttpResponse.json({ code: 200, message: '终止成功', data: inst });
  }),
