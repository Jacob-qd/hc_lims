...
export const AIPredictionPage: React.FC = () => {
  const [data, setData] = useState<LooseAny>(null);
  ...
  const areaConfig = data?.predictions ? {
    data: data.predictions.flatMap((p: LooseAny) => [
    ...
    areaStyle: (datum: LooseAny) => datum.type === '上限' || datum.type === '下限' ? { fillOpacity: 0.1 } : { fillOpacity: 0.2 },
  } : null;

  if (loading) return <Spin size="large" style={{ display: 'block', margin: '40px auto' }} />;

  const activeAlerts = data?.alerts?.filter((a: LooseAny) => a.status === 'active') || [];
  const criticalCount = activeAlerts.filter((a: LooseAny) => a.level === 'critical').length;
  const warningCount = activeAlerts.filter((a: LooseAny) => a.level === 'warning').length;
  ...
  return (
    <div>
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col xs={24} sm={8}><Card><Statistic title="已解决" value={(data?.alerts?.filter((a: LooseAny) => a.status === 'resolved').length) || 0} valueStyle={{ color: '#52c41a' }} prefix={<RiseOutlined />} /></Card></Col>
      </Row>
