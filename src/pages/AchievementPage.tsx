...
export const AchievementPage: React.FC = () => {
  const [pubs, setPubs] = useState<Achievement[]>([]);
  ...
  const filtered = pubs.filter((p: Achievement) => {
    const matchesSearch = p.title.includes(search) || p.authors.includes(search) || p.journal.includes(search);
    const matchesType = typeFilter ? p.type === typeFilter : true;
    const matchesYear = yearFilter ? p.year === yearFilter : true;
    return matchesSearch && matchesType && matchesYear;
  });

  const stats = {
    total: pubs.length,
    papers: pubs.filter((p: Achievement) => p.type === '论文').length,
    patents: pubs.filter((p: Achievement) => p.type === '专利').length,
    published: pubs.filter((p: Achievement) => p.status === 'published').length,
  };

  const yearStats = pubs.reduce((acc: Record<number, number>, p: Achievement) => {
  ...
  const handleCreate = (values: Record<string, unknown>) => {
    ...
    form.resetFields();
    setPubs(prev => [...prev, { ...values, id: `pub${prev.length + 1}`, status: 'draft' }]);
  };
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={6}><Card size="small"><Statistic title="成果总数" value={stats.total} prefix={<BarChartOutlined />} /></Card></Col>
      ...
      <Card>
        <Space style={{ marginBottom: 16 }}>
          ...
          <Select placeholder="类型" style={{ width: 120 }} allowClear onChange={v => setTypeFilter(v)}>
            {Object.entries(typeColors).map(([k, col]) => <Select.Option key={k} value={k}><Tag color={col}>{k}</Tag></Select.Option>)}
          </Select>
          <Select placeholder="年份" style={{ width: 100 }} allowClear onChange={v => setYearFilter(v ? Number(v) : null)}>
        ...
        <Table
          ...
          columns={[
            { title: '标题', dataIndex: 'title', render: (t: string, r: Achievement) => <a onClick={() => { setSelected(r); setDrawer(true); }}>{t}</a> },
      ...
      <Drawer title={selected?.title} open={drawer} onClose={() => { setDrawer(false); setSelected(null); }} width={560}>
        {selected && (<>
          ...
          <Tabs style={{ marginTop: 16 }} items={[
              ...
              key: 'export', label: '数据导出', children: (
                <Space direction="vertical" style={{ width: '100%' }}>
                  {[{ fmt: 'CSV', desc: '原始数据表格' }, { fmt: 'Excel', desc: '论文作图数据' }, { fmt: 'PDF', desc: '完整实验报告' }].map(f => (
                    <Card key={f.fmt} size="small" hoverable>
                      <Row justify="space-between">
                        <Col>
                          <Space><FilePdfOutlined /><Text strong>{f.fmt}</Text></Space>
                          <br /><Text type="secondary">{f.desc}</Text>
                        </Col>
                        <Col>
            ...
            {
              key: 'stats', label: '年度统计', children: (
                <div>
                  {Object.entries(yearStats).sort((a, b) => Number(b[0]) - Number(a[0])).map(([year, count]) => (
                    <div key={year} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                      <Text style={{ width: 50 }}>{year}年</Text>
                      <div style={{ flex: 1, background: '#f0f0f0', borderRadius: 4, overflow: 'hidden' }}>
                        <div style={{ width: `${Math.min((count / stats.total) * 300, 100)}%`, background: '#1677ff', height: 16, borderRadius: 4 }} />
                      </div>
                      <Text strong>{count}篇</Text>
                    </div>
                  ))}
                </div>
              )
      ...
      <Modal title="新增成果" open={createModal} onOk={() => form.submit()} onCancel={() => { setCreateModal(false); form.resetFields(); }} width={560}>
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          ...
          <Row gutter={16}>
            <Col span={12}><Form.Item name="journal" label="期刊/专利号"><Input /></Form.Item></Col>
            <Col span={12}><Form.Item name="year" label="年份"><Input type="number" /></Form.Item></Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="authors" label="作者"><Input /></Form.Item></Col>
            <Col span={12}><Form.Item name="doi" label="DOI"><Input /></Form.Item></Col>
          </Row>
