...
import {
  ...
  Timeline, Descriptions, Divider, Row, Col, Statistic, Badge, Tabs, Upload,
  List, Typography, Empty,
} from 'antd';
import {
  PlusOutlined, DeleteOutlined, EyeOutlined, FileTextOutlined,
      ...
      <Modal title={selectedEntry?.title} open={detailOpen} onCancel={() => setDetailOpen(false)} footer={null} width={720}>
        {selectedEntry && (
          <Tabs activeKey={activeDetailTab} onChange={setActiveDetailTab}>
            <TabPane tab={<span><FileTextOutlined /> 记录内容</span>} key="content">
            ...
            <TabPane tab={<span><CommentOutlined /> 批注协作 ({selectedEntry.annotations.length})</span>} key="annotations">
            ...
            <TabPane tab={<span><HistoryOutlined /> 审计追踪 ({selectedEntry.auditTrail.length})</span>} key="audit">
            ...
            <TabPane tab={<span><FileTextOutlined /> 版本历史 ({selectedEntry.versions.length})</span>} key="versions">
            ...
            <TabPane tab={<span><SignatureOutlined /> 签名信息 ({selectedEntry.signatures.length})</span>} key="signatures">
