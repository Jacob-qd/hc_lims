...
export const InventoryPage: React.FC = () => {
  const [items, setItems] = useState<LooseAny[]>([]);
  const [prs, setPrs] = useState<LooseAny[]>([]);
  ...
  const filtered = items.filter((i: LooseAny) => i.name.includes(search) || i.code.includes(search) || i.supplier.includes(search));
  const stats = { total: items.length, low: items.filter((i: LooseAny) => i.status === 'low' || i.status === 'out').length, expiring: items.filter((i: LooseAny) => i.status === 'expiring').length, pendingPr: prs.filter((p: LooseAny) => p.status === 'pending_approval').length };
      ...
      <Tabs defaultActiveKey="inventory" items={[
        { key: 'inventory', label: '库存列表', children: (
          ...
          <Table dataSource={filtered} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} columns={[
            ...
            { title: '名称', dataIndex: 'name', render: (n: string, r: Record<string, LooseAny>) => <a onClick={() => { setSelected(r); setDrawer(true); }}>{n}</a> },
            { title: '分类', dataIndex: 'category', render: (c: string) => <Tag color={categoryColors[c]}>{c}</Tag> },
            { title: '批号', dataIndex: 'batchNo' },
            { title: '库存', dataIndex: 'stock', render: (s: number, r: Record<string, LooseAny>) => <Text type={s <= r.safetyStock ? 'danger' : undefined}>{s}{r.unit}</Text> },
            { title: '安全库存', dataIndex: 'safetyStock', render: (s: number, r: Record<string, LooseAny>) => <Text type={r.stock <= s ? 'danger' : undefined}>{s}{r.unit}</Text> },
          ...
          <Card title="到期预警 (30天内)" size="small" style={{marginTop:16}}>
            <Row gutter={16}>
              {filtered.filter((i: LooseAny) => i.status === 'expiring' || i.status === 'low' || i.status === 'out').slice(0,4).map((i: LooseAny) => (
                <Col span={6} key={i.id}>
              ...
              {filtered.filter((i: LooseAny) => i.status === 'expiring' || i.status === 'low').length === 0 && <Col span={24}><Text type="secondary">暂无到期预警</Text></Col>}
            </Row>
          </Card>
        </Card>
        )},
        ...
        { key: 'purchase', label: '采购申请', children: (
          <Card><Table dataSource={prs} rowKey="id" columns={[
            ...
            { title: '操作', render: (_: string, r: Record<string, LooseAny>) => <Button type="link" size="small" onClick={() => { setSelected(r); setPrDrawer(true); }}>查看</Button> },
