...
const LJChart: React.FC<{data: LooseAny}> = ({data}) => {
  ...
  const vals = pts.map((p: LooseAny) => p.value);
  ...
  const pd = pts.map((p: Record<string, LooseAny>,i:number) => `${i===0?'M':'L'}${sx(i)},${sy(p.value)}`).join('');
  return <svg viewBox={"0 0 "+w+" "+h} style={{width:'100%',height:280}}>
    ...
    {pts.map((p: Record<string, LooseAny>,i:number) => {
    ...
    {pts.filter((_: Record<string, LooseAny>,i:number) => i%3===0||i===pts.length-1).map((p: LooseAny) => {
...
export const QualityPage: React.FC = () => {
  ...
  const [chartData, setChartData] = useState<LooseAny>(null);
    ...
    <div>
      ...
      <Tabs defaultActiveKey="overview" items={[
          ...
          <Row gutter={16}>
            <Col span={24} style={{ marginBottom: 16 }}>
              <Card title="Levey-Jennings 控制图">
                {chartData && (chartData as LooseAny).points ? <LJChart data={chartData} /> : <Text type="secondary">加载中...</Text>}
              </Card>
            </Col>
            <Col span={24}>
        ...
        { key: 'westgard', label: 'Westgard规则分析', children: (
          <WestgardAnalyzer
            data={(chartData as LooseAny)?.points?.map((p: LooseAny) => ({ value: p.value, date: p.date, batch: p.batch || '' })) || []}
            mean={(chartData as LooseAny)?.mean || 0}
            sd={(chartData as LooseAny)?.sd || 1}
            analyte="COD"
          />
        )},
            ...
            <Tabs items={[
              {key:'detail', label:'偏差详情', children:<>
                <Descriptions column={1} bordered size="small">
                  ...
                  <Descriptions.Item label="责任人">{(selectedDev as LooseAny)['责任人'] || '-'}</Descriptions.Item>
              ...
              {key:'audit', label:'审计追踪', children:<Table dataSource={[
                {time:selectedDev.foundDate+' 09:15',user:(selectedDev as LooseAny)['责任人']||'-',action:'创建偏差',detail:'偏差编号 '+selectedDev.no},
