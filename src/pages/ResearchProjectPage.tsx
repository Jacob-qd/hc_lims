...
export const ResearchProjectPage: React.FC = () => {
  ...
  useEffect(() => {
    window.fetch('/api/v1/research/projects')
      .then(r => r.json())
      .then(json => setProjects(json.data.list))
      .catch(() => message.error('加载失败'))
      .finally(() => setLoading(false));
  }, []);

  const filtered = projects.filter((p: ResearchProject) => p.name.includes(searchText) || p.pi.includes(searchText) || p.no.includes(searchText));
