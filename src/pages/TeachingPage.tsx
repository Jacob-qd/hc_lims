...
export const TeachingPage: React.FC = () => {
  ...
  const stats = { total: courses.length, students: courses.reduce((s: number, c: Course) => s + c.students, 0), experiments: courses.reduce((s: number, c: Course) => s + c.experiments, 0), active: courses.filter((c: Course) => c.status === 'active').length };
